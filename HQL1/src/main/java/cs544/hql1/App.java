...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Query query = session.createQuery("select F from Flight as F " +
                "join F.origin as O " +
                "join F.destination as D " +
                "join F.airplane as A " +
                "where A.capacity > :limitLowCapacity " +
                "and O.country = :origingCountry " +
                "and not D.country = :destinationCountry");

        query.setParameter("limitLowCapacity", 500);
        query.setParameter("origingCountry", "USA");
        query.setParameter("destinationCountry", "USA");
        ...
        query = session.createQuery("select distinct A from Airline as A " +
                "join A.flights as F " +
                "join F.airplane as airp " +
                "where airp.model = :airplaneModel");

        query.setParameter("airplaneModel", "A380");
        ...
        query = session.createQuery("select F from Flight as F " +
                "join F.airplane as airp " +
                "where airp.model = :airplaneModel " +
                "and not F.airline.name = :airlineName");

        query.setParameter("airplaneModel", "747");
        ...
        query = session.createQuery("from Flight as F " +
                "where F.departureDate = '2009-08-7' " +
                "and F.departureTime < '12:00:00'");
